...
public class Regularization {
	...
	//public static void main(String[] args) throws InterruptedException {
		...
		for(int i=1; i<=b1; i++)
		{
			if(b1<=6)
			{
				int j = driver.findElements(By.cssSelector
						("li.row.card.buzzerPointer.pinkBlockQuote.ticketListCard div:nth-child(1) span b")).size();
				for(int k=0; k<=j-1; k++)
				{
				String Q_ID = driver.findElements(By.cssSelector
						("li.row.card.buzzerPointer.pinkBlockQuote.ticketListCard div:nth-child(1) span b")).get(k).getText(); //Query ID
